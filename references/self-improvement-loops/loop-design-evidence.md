...
## Loop termination and resource bounding (added 2026-08-18, not from the original evidence index)

The two entries below were retrieved and verified directly against the papers on 2026-08-18, independent of `researcher/claims/index.jsonl` — no `claim-*` ID is assigned, consistent with every other unindexed citation in this file's "External resources" style entries elsewhere in the skill.

Hou, Wang, Zhao, Wang, "When Agents Do Not Stop: Uncovering Infinite Agentic Loops in LLM Agents", arXiv 2607.01641, 2026.
https://arxiv.org/abs/2607.01641

- Classifies a loop's continuation controller as deterministic, model-controlled, tool-controlled, external-state-controlled, exception-controlled, or mixed. Only the deterministic kind is a verifiable bound; the others depend on a component (the model, a tool, external state) known to drift or misreport "done."
- IAL-Scan, their static analyzer, checks whether a declared bound (`max_iterations`, `max_retry`, `recursion_limit`, a timeout) actually covers the feedback path it claims to bound, rather than only checking that a bound exists somewhere in the code.
- Results: 68 confirmed infinite-agentic-loop failures across 47 of 6,549 scanned repositories, 91.9% precision. The dominant failure shape was a local bound that did not cover an outer loop — a tool-call retry cap with no cap on the surrounding call count, growing message history with no `max_tool_calls` — not the absence of any bound at all.
- Relevance to this skill: a self-editing proposer's bounded edits routinely touch exactly this kind of wrapper and reentry code (tool dispatch, retries, agent-as-tool handoffs), and the standard two-split acceptance gate (task-performance regression) does not check for this failure mode.

Ye and Tan, "Agent Contracts: A Formal Framework for Resource-Bounded Autonomous AI Systems", arXiv 2601.08815, 2026.
https://arxiv.org/abs/2601.08815

- Formalizes multi-dimensional resource budgets (tokens, tool/API calls, iterations, wall-clock duration, cost) with an existential termination rule: a breach on any single dimension ends the run, not just an aggregate.
- Two enforcement modes: soft (budget-aware prompting, cooperative) and hard (an external monitor tracking consumption after every action, halting independent of the agent's cooperation). Reports "token elasticity" — agents documented ignoring or exceeding limits stated only through the prompt, i.e. under soft enforcement alone.
- Delegation formalism for multi-agent budgets: sum of child budgets must not exceed the parent budget, allocated proportionally, equally, or by negotiation with a 10-15% reserve, with unused budget returned to a shared pool as agents complete. A 3-agent research-pipeline experiment reported zero conservation-law violations across 50 trials.
- Documented incidents: hard enforcement detected and halted an agent that had consumed 56K tokens against a 40K allocation, mid-run — this one is the paper's own experiment. Separately, the paper's introduction cites a third-party report (not its own data) of two agents stuck in a recursive clarification loop with no stop condition running for 11 days and producing a $47,000 bill before detection — offered as motivation, not as a case the authors measured themselves.
- Also reports a strategy-mode experiment showing a reproducible quality-resource tradeoff curve: 70% task success at 0 reasoning tokens versus 86% success at 1,519 tokens (roughly +75% token spend for +16 points of success, p≈0.05) — a template for reporting resource-quality tradeoffs rather than a single score at an unstated budget.
