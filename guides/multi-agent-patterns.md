> Segment of the `agent-oracle` skill, from the upstream `multi-agent-patterns` skill.
...
There is a formal argument for why single-agent baselines win by default, not just an empirical caution. Under a fixed thinking-token budget, single-agent systems are more information-efficient than multi-agent ones by a Data-Processing-Inequality argument: splitting a task across agents adds serialization and re-summarization steps that can only lose information relative to a single agent reasoning over the same tokens directly (Tran and Kiela, "Single-Agent LLMs Outperform Multi-Agent Systems... Under Equal Thinking Token Budgets," arXiv 2604.02460). Multi-agent systems became competitive only when a single agent's context utilization was already degraded, or when more total compute was spent, not at equal budgets. The paper also flags a measurement trap worth knowing before trusting a baseline comparison: budget-control artifacts in some vendor APIs (their example is Gemini 2.5) can inflate apparent multi-agent gains, so a "measure against single-agent baselines" comparison needs to be token-normalized, not just present.

**Resource Contracts for Delegated Budgets**
Give delegation a formal budget contract, not just a shared ceiling. Sum of every child agent's budget must not exceed the parent's budget — token, tool-call, iteration, and wall-clock dimensions all included, not just token count. Allocate by one of three rules: proportionally to expected subtask cost, equally across workers, or by negotiation, and hold back a 10-15% reserve rather than allocating 100% up front. Return unused budget to a shared pool as agents complete, so an early-finishing worker's slack is available to a worker still running (Ye and Tan, "Agent Contracts: A Formal Framework for Resource-Bounded Autonomous AI Systems," arXiv 2601.08815). Their 3-agent research-pipeline experiment reported zero conservation-law violations across 50 trials, and one case of the enforcement layer catching and halting a worker mid-run after it exceeded its allocation. This gives the supervisor and hierarchical patterns below a concrete delegation contract, rather than only "budget for overhead."
...
9. Default to single-agent under a fixed token budget; multi-agent is competitive only when context utilization is already degraded or more total compute is spent, and any comparison against a single-agent baseline must be token-normalized, not just present.
10. Delegate budgets formally: sum of child budgets must not exceed the parent's, across every resource dimension, with a 10-15% reserve and unused budget returned to a shared pool.
...
9. **Multi-agent as the default, not the exception** — Under an equal thinking-token budget, single-agent systems are more information-efficient by a Data-Processing-Inequality argument: splitting a task across agents adds serialization and re-summarization that can only lose information (arXiv 2604.02460). Justify multi-agent before building, against a token-normalized single-agent baseline, not after. Watch for vendor API budget-control artifacts inflating apparent multi-agent gains in that comparison.
...
- `guides/evaluation.md`: measuring whether multiple agents improve outcomes after coordination cost.
...
- Ye and Tan, "Agent Contracts: A Formal Framework for Resource-Bounded Autonomous AI Systems" (arXiv 2601.08815) - Formal budget-delegation contract for supervisor and hierarchical topologies
- Tran and Kiela, "Single-Agent LLMs Outperform Multi-Agent Systems... Under Equal Thinking Token Budgets" (arXiv 2604.02460) - Information-theoretic argument for single-agent as the default under fixed budgets
