> Segment of the `agent-oracle` skill, from the upstream `self-improvement-loops` skill.
...
- Governance of a single autonomous loop that does not modify itself: locked and editable surfaces, durable logs, rollback, novelty gates, PR preparation, and human approval boundaries belong to `guides/harness-engineering.md`. That skill defines the control surfaces; this skill defines what happens when the surfaces themselves become the optimization target.
- Building the evaluator, regression suite, or quality gates that score candidates: `guides/evaluation.md`.
...
Enforcement itself splits into two modes, and a self-improving loop needs the hard one. Soft enforcement — budget-aware prompting, cooperative and best-effort — is exactly the "constraints stated in prompt text get evolved away" failure above, formalized as "token elasticity": agents document ignoring or exceeding limits communicated only through the prompt. Hard enforcement is an external monitor that tracks consumption after every action and halts independent of the loop's cooperation. A working example of the difference: an external monitor detected an agent that had exceeded a 40K-token allocation by consuming 56K, and halted it mid-run — something a prompt-stated budget would not have caught, because by definition it depends on the very component that overran it (Ye and Tan, "Agent Contracts: A Formal Framework for Resource-Bounded Autonomous AI Systems," arXiv 2601.08815).
...
The two-split gate checks task-performance regression. It does not check structural runaway risk, and a self-editing proposer's edits are exactly the shape that hides one: bounded proposals routinely touch wrapper and reentry code — tool dispatch, retry logic, agent-as-tool handoffs — where a termination bound can be weakened or removed without moving the held-in or held-out score at all (Hou, Wang, Zhao, and Wang, "When Agents Do Not Stop: Uncovering Infinite Agentic Loops in LLM Agents," arXiv 2607.01641). Add a bound-coverage check as a separate, mandatory gate for any candidate touching loop, retry, recursion, or handoff code: classify the edited continuation controller and verify the bound still covers the outer feedback path. A candidate that passes the two-split gate but fails bound coverage is still a rejected candidate.
...
13. Before merging any self-edit that touches loop, retry, recursion, or handoff code, verify it doesn't remove or weaken a termination bound — the two-split acceptance gate checks task performance, not structural runaway risk.
...
10. **Two-split gate blind to loop safety**: A self-edit can pass held-in and held-out performance checks while removing a termination bound buried in wrapper or reentry code — exactly the shape of edit a self-editing proposer produces, and exactly the shape the acceptance gate above was never designed to catch (arXiv 2607.01641). Add a bound-coverage check as a separate, mandatory gate rather than trusting performance non-regression to cover it.
...
- Hou, Wang, Zhao, Wang, "When Agents Do Not Stop: Uncovering Infinite Agentic Loops in LLM Agents" (arXiv 2607.01641) - Static bound-coverage analysis for self-edits touching loop or handoff code
- Ye and Tan, "Agent Contracts: A Formal Framework for Resource-Bounded Autonomous AI Systems" (arXiv 2601.08815) - Hard versus soft budget enforcement for the outside-the-loop invariant

Numeric, benchmark, volatile, or vendor-performance claims in this skill carry inline `claim-*` IDs backed by `researcher/claims/index.jsonl`. Detailed numbers live in the dated reference file. The two arXiv sources added above are cited inline without a `claim-*` tag — they were verified directly against the papers, not sourced through that index.
