> Segment of the `agent-oracle` skill, from the upstream `long-horizon-prompting` skill.
...
- Agent topology, supervisor versus swarm choice, handoff protocols, and coordination mechanics: `guides/multi-agent-patterns.md`. That skill owns the architecture; this skill owns the words that steer it.
- Runtime control surfaces, locked evaluators, rollback, durable logs, and approval boundaries around an autonomous loop: `guides/harness-engineering.md`. Constraints that must survive optimization pressure belong in the harness, not the prompt.
- Building the evaluator, regression suite, or deterministic quality gates a run is scored by: `guides/evaluation.md`.
...
- Loops that modify their own harness or prompts: `guides/self-improvement-loops.md`.
...
- Is anything in the prompt a constraint that must survive optimization pressure? Move it to the harness (`guides/harness-engineering.md`); prompt-stated constraints are advisory.
...
8. **Prompt-stated budgets decay**: A budget or reminder stated once loses force as the trajectory grows; re-inject budget and verified-progress state periodically from outside the loop (claim-long-horizon-give-up-drift). Independent corroboration outside this skill's own evidence base: agents document "token elasticity", ignoring or exceeding resource limits communicated only through the prompt (Ye and Tan, "Agent Contracts," arXiv 2601.08815) — the same failure, from a harness-design angle rather than a prompting one.
...
Internal references:
- [Annotated CDC prompt](./references/long-horizon-prompting/cdc-prompt-annotated.md) - The full published prompt with element-by-element annotation and provenance
- [Vendor guidance](./references/long-horizon-prompting/vendor-guidance.md) - Dated OpenAI and Anthropic long-horizon and multi-agent prompting doctrine with sources
- [Research evidence](./references/long-horizon-prompting/research-evidence.md) - Dated academic findings behind each brief element
- [Task brief template](./references/long-horizon-prompting/task-brief-template.md) - Reusable pseudo-formal brief template and pre-launch evaluation rubric
