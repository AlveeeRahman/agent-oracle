> Segment of the `agent-oracle` skill, from the upstream `evaluation` skill.
...
- Designing autonomous control surfaces, novelty gates, rollback, or PR approval boundaries: `guides/harness-engineering.md`.
...
**Evaluate Stopping Behavior, Not Just Output Quality**

Output-quality rubrics can pass an agent that is quietly failing to stop correctly: submitting duplicates, declaring false completion, or drifting off the goal as a task scales. This is a distinct failure mode from the accuracy/completeness dimensions above and needs its own measurement, not an inference from a pass/fail flag. Cai et al. define Quantitative Goal Persistence (QGP) and a benchmark, PushBench, that measures duplicate submissions, false completion, and progress drift directly (arXiv 2605.23574). Their finding is scale-dependent in a way a single-difficulty test set would miss entirely: frontier agents solved most 50-artifact tasks but dropped to 3 of 9 successes at 100 artifacts. Include a scale-stratified goal-persistence check alongside the complexity strata already recommended below, not only a separate difficulty axis.
...
- Goal persistence: Completes without duplicate submissions, false-completion claims, or progress drift as task scale grows (weight for long-running or large-scope tasks)
...
9. For long-running or large-scope agents, measure goal persistence (duplicate submissions, false completion, progress drift) at multiple scale levels, not just at one fixed task size
...
9. **Testing goal persistence at only one scale**: An agent that reliably completes a 50-item task can drop sharply on a 100-item version of the same task — one documented case went from mostly passing to 3 of 9 successes (arXiv 2605.23574). A single-scale test set will not surface this; stratify goal-persistence checks by scale the same way you already stratify by complexity.
...
- `guides/harness-engineering.md`: locked evaluators, editable surfaces, rollback, and human approval boundaries.
...
- `guides/multi-agent-patterns.md`: evaluating coordination quality and parallelization trade-offs.
...
- Cai et al., "Push Your Agent" (arXiv 2605.23574) - Quantitative Goal Persistence and PushBench, measuring duplicate submissions, false completion, and progress drift at scale
